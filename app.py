...
alpha_vantage_key = "9OKPDVAM9DQLJJTR"

if custom_ticker:
    with st.sidebar:
        with st.spinner(f"Verifying ticker {custom_ticker.upper()}..."):
            if verify_ticker(custom_ticker.upper()):
...
if enable_sentiment:
    ...
    openrouter_key = st.sidebar.text_input(
        ...
        value="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",
        type="password",
...
if enable_comparison:
    comparison_tickers = st.sidebar.multiselect(
        ...
        options=[stock['symbol'] for stock in popular_stocks if stock['symbol'] != selected_stock],
        default=[],
        help="Select up to 3 stocks to compare with the primary stock"
    )
    # Limit to 3 comparison stocks
    if len(comparison_tickers) > 3:
        comparison_tickers = comparison_tickers[:3]
        st.sidebar.warning("Maximum of 3 comparison stocks allowed. Using the first 3 selected.")
st.sidebar.markdown('</div>', unsafe_allow_html=True)
    ...
    if enable_sentiment and fetch_button:
        with st.spinner(f"Analyzing sentiment for {selected_stock}..."):
            ...
            if sentiment_data:
                ...
                # Chat interface
                st.markdown(get_chart_container_start_html("Ask about this stock"), unsafe_allow_html=True)
                if "chat_history" not in st.session_state:
                    st.session_state.chat_history = []
                for message in st.session_state.chat_history:
                    with st.chat_message(message["role"]):
                        st.markdown(message["content"])
                user_question = st.chat_input("Ask something about this stock...")
                if user_question:
        ...
        with risk_container:
            ...
            if enable_benchmark:
                with st.spinner(f"Fetching benchmark data ({benchmark_ticker})..."):
                    benchmark_data, error = get_stock_data(
                        benchmark_ticker, 
                        period=selected_period,
                        alpha_vantage_key=alpha_vantage_key
                    )
...
with tab5:
    ...
    if enable_comparison and fetch_button:
        if not comparison_tickers:
            ...
            with st.spinner(f"Fetching comparison data for {', '.join(comparison_tickers)}..."):
                ...
                if 'stock_data' not in st.session_state:
                    main_data, error = get_stock_data(
                        selected_stock, 
                        period=selected_period,
                        alpha_vantage_key=alpha_vantage_key
                    )
                ...
                # If main data is available, fetch comparison data
                if main_data is not None and not main_data.empty:
                    # Store comparative data in a dictionary
                    ...
                    for comp_ticker in comparison_tickers:
                        ...
                        if comp_error:
                            st.warning(f"Error fetching data for {comp_ticker}: {comp_error}")
                        elif comp_data is None or comp_data.empty:
                            st.warning(f"No data available for {comp_ticker}")
                        else:
                            comparative_data[comp_ticker] = comp_data
                    
                    # Use the comprehensive comparative analysis UI
                    if len(comparative_data) > 1:  # Need at least 2 stocks to compare
                        render_comparative_analysis_ui(comparative_data)
                    else:
                        st.warning("Need at least two stocks with valid data for comparison. Please select more comparison stocks.")
                else:
                    st.error(f"No data available for the main stock {selected_stock}")
    elif not enable_comparison:
